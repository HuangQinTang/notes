...
- ![none](.\img\image-20220117201006909.png)
  ...
  - **4.遍历bucket查找**
    ...
    ![img](.\img\f39e10e1474fda593cbca86eb0c517e2.png)
...
- **底层数据结构：**
  ...
  ![image-20220729001246841](.\img\image-20220729001246841.png)
...
**P（Processor）：虚拟(逻辑态)处理器，M执行G所需要的资源和上下文，处理用户级代码逻辑的处理器。只有将 P 和 M 绑定，才能让 P 的 runq 中的 G 真正运行起来。P 的数量决定了系统内最大可并行的 G 的数量，**P的数量受本机的CPU核数影响，可通过环境变量$GOMAXPROCS或在runtime.GOMAXPROCS()来设置，默认为CPU核心数。
...
- 若本地队列也满了，则将本地队列中的 G 拿出一半(防止饥饿，会拉取前半部分，为了防止不同cpu读取同个缓存行，会打乱循序放到全局)，放到全局队列中，生产结束。

![img](.\img\33653a35dca9472fd1e487d3a94557c6.writebug)
...
![img](.\img\image-20220419153014030.png)
...
![img](.\img\image-20220419152721948.png)
...
![img](.\img\image-20220422214355669.png)

点击 `view trace` 能够看见可视化的调度流程：

![img](.\img\image-20220422213604926.png)

![img](.\img\image-20220422210738598.png)
...
![img](.\img\image-20220422211223494.png)
...
![img](.\img\image-20220422212719433.png)
...
![img](.\img\image-20220501185553347.png)
...
![img](.\img\watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzQzMTg4NzQ0,size_16,color_FFFFFF,t_70.png)
...
![img](.\img\image-20220501185248901.png)
...
![img](.\img\image-20220502132935164.png)

以T2结构体为例，实际存储数据的只有13字节，但实际用了24字节，浪费了11个字节：

![img](.\img\image-20220502133003644.png)

以T3结构体为例，实际存储数据的只有13字节，但实际用了16字节，浪费了3个字节：

![img](.\img\image-20220502133303337.png)
...
![img](https://image-1302243118.cos.ap-beijing.myqcloud.com/imgcdn/image-20220503232512072.png)
...
![img](https://image-1302243118.cos.ap-beijing.myqcloud.com/imgcdn/image-20220504185906969.png)
